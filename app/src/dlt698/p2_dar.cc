...
// 为了节约内存，const部分集中在一起
// 固定部分
static const PcutItemFix kPartFix[kP2DarPartNum] = {
	// name len offset valid
	{ kP2DarName, LenDar, OffsetDar, ValidDar },
};
	

static const PcutItem kPartItemsPattern[kP2DarPartNum] = {
	PCUT_ITEM_NO_SUB(&kPartFix[kP2DarPartIxDar]),
};
static void PcutItemsInit(PcutItem items[kP2DarPartNum])
{
...
cp_t P2DarPcutOpen(P2DarPcut *m)
{
	...
	ifer(PcutOpen(&m->base, m->items, kP2DarPartNum));
	return 0;
}
