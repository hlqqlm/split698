/*
This program is licensed under the GPLv2 License:
Copyright 2023 Huanglin <hlqqlm@qq.com>


This file is part of Split698.

Split698 is free software: you can redistribute it and/or modify it under the
terms of the GNU General Public License as published by the Free Software
Foundation, either version 2 of the License, or (at your option) any later
version.

Split698 is distributed in the hope that it will be useful, but WITHOUT ANY
WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS FOR
A PARTICULAR PURPOSE. See the GNU General Public License for more details.

You should have received a copy of the GNU General Public License along with
Split698. If not, see <https://www.gnu.org/licenses/>.

20230822-----------------------------------------------------------------------
huanglin 创建.
用来描述dlt698_45中的CSD
列选择描述符CSD（Column Selection Descriptor）的数据类型定义见表19
*/

// var
#include "p2_oad.h"
#include "p2_road.h"

#include "p2_csd.h"
#include "p2_csd.xcp.h"


#define TEST_EN				(0)
#define kChoiceNum			(kP2CsdNum)


// {{{ choice
// 必须按大小顺序排
static const P2Choice kChoiceList[kChoiceNum] = {
	// choice	name
	{ kP2CsdOad, "oad" },	// 对象属性描述符 [0] OAD，
	{ kP2CsdRoad, "road" },	// 记录型对象属性描述符 [1] ROAD
};
int P2Csd2Ix(uint8_t choice)
{
	return P2Choice2Ix(kChoiceList, kChoiceNum, choice);
}
uint8_t P2CsdIx2Value(int choice_ix)
{
	return P2ChoiceIx2Value(kChoiceList, kChoiceNum, choice_ix);
}
cp_t P2CsdIxValid(int choice_ix)
{
	return P2ChoiceIxValid(kChoiceNum, choice_ix);
}
cp_t P2CsdValid(uint8_t choice)
{
	return P2ChoiceValid(kChoiceList, kChoiceNum, choice);
}
const char *P2CsdStr(uint8_t choice)
{
	return P2ChoiceStr(kChoiceList, kChoiceNum, choice);
}
//}}}


//{{{ var_factory_info
static const P2OadPcut kP2OadPcutDefVar = kP2OadPcutDef;
static const P2RoadPcut kP2RoadPcutDefVar = kP2RoadPcutDef;
static const PcutFactoryInfo kVarFactoryInfoList[kChoiceNum] = {
	// name		size	init	derive_open		derive_close
	{ kP2OadName, sizeof(P2OadPcut), &kP2OadPcutDefVar, P2OadPcutOpenBase, P2OadPcutCloseBase },	// 对象属性描述符 [0] OAD，
	{ kP2RoadName, sizeof(P2RoadPcut), &kP2RoadPcutDefVar, P2RoadPcutOpenBase, P2RoadPcutCloseBase },	// 记录型对象属性描述符 [1] ROAD
	// kPcutFactoryInfoDef("example"),	// 选择方法 2 [2] Selector2，
};
//}}}


//{{{ pcut
cp_t P2CsdPcutOpen(P2CsdPcut *m)
{
	return P2ChoicePcutOpen(&m->choice, kP2CsdNameChoice, kChoiceList, kChoiceNum, kVarFactoryInfoList);
}
cp_t P2CsdPcutClose(P2CsdPcut *m)
{
	return P2ChoicePcutClose(&m->choice);
}
cp_t P2CsdPcutValid(const P2CsdPcut *m)
{
	return P2ChoicePcutValid(&m->choice);
}
//}}}


//{{{ fill
cp_t P2CsdFillInit(Pfill *m, uint8_t choice, Pfill *sub_var)
{
	const P2ChoiceFillItemChoice kFiChoice = kP2ChoiceFillItemChoiceDef(kP2CsdNameChoice, PfillItemOffsetFix, 0, choice);
	ifbr(NULL != PfillItemFactory(m, sizeof(kFiChoice), &kFiChoice));

	const PfillItem kFiVar = PFILL_ITEM(kP2CsdNameVar, PfillItemOffsetFix, PfillItemProcessBySub, 1, sub_var);
	ifbr(NULL != PfillItemFactory(m, sizeof(kFiVar), &kFiVar));

	return 0;
}
//}}}


//{{{ test
#if TEST_EN > 0
static cp_t TestPcut(void)
{
	return 0;
}
static cp_t TestQpack(void)
{
	return 0;
}


static const QtestItem kTestItem[] = {
	TestPcut,
	TestQpack,
};
#define kTestItemNum	(sizeof(kTestItem)/sizeof(kTestItem[0]))
cp_t P2CsdTest(QTEST_ARG)
{
	return QtestTest(__FILE__, run_times, kTestItem, kTestItemNum, verbose);
}

#else
cp_t P2CsdTest(QTEST_ARG)
{
	return 0;
}
#endif
//}}}

