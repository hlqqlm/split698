...
// 为了节约内存，const部分集中在一起
// 固定部分
static const PcutItemFix kPartFix[kP2OctetStringPartNum] = {
...
static const PcutItem kPartItemsPattern[kP2OctetStringPartNum] = {
	// PCUT_ITEM_NO_SUB(&kPartFix[kP2OctetStringPartIxDatatype]),
	PCUT_ITEM_NO_SUB(&kPartFix[kP2OctetStringPartIxVarlen]),
	PCUT_ITEM_NO_SUB(&kPartFix[kP2OctetStringPartIxContent]),
};
static void PcutItemsInit(PcutItem items[kP2OctetStringPartNum])
{
...
cp_t P2OctetStringPcutOpen(P2OctetStringPcut *m)
{
	...
	ifer(PcutOpen(&m->base, m->items, kP2OctetStringPartNum));
	return 0;
}
...
static cp_t TestPcutDatatypeNotExist(void)
{
	...
	//ifbr(0 == PcutIxLen(m, kP2OctetStringPartIxDatatype, whole));
	ifbr(1 == PcutIxLen(m, kP2OctetStringPartIxVarlen, whole));
	ifbr(21 == PcutIxLen(m, kP2OctetStringPartIxContent, whole));
	ifbr(22 == PcutIxLen(m, kPcutIxAll, whole));

	//ifbr(0 == PcutIxOffset(m, kP2OctetStringPartIxDatatype, whole));
	//ifbr(0 == PcutIxOffset(m, kP2OctetStringPartIxVarlen, whole));
	ifbr(1 == PcutIxOffset(m, kP2OctetStringPartIxContent, whole));
	ifbr(22 == PcutIxOffset(m, kPcutIxAll, whole));

	//ifer(PcutIxValid(m, kP2OctetStringPartIxDatatype, whole));
	ifer(PcutIxValid(m, kP2OctetStringPartIxVarlen, whole));
	ifer(PcutIxValid(m, kP2OctetStringPartIxContent, whole));
...
static cp_t TestPcutDatatypeExist(void)
{
	...
	ifbr(1 == PcutIxLen(m, kP2DatatypeWithContentPartIxDatatype, whole));
	ifbr(7 == PcutIxLen(m, kP2DatatypeWithContentPartIxContent, whole));
	const char *os_mem = PcutIxPtrConst(m, kP2DatatypeWithContentPartIxContent, whole);		// const版本
	ifbr(1 == PcutIxLen(&pcut.base, kP2OctetStringPartIxVarlen, os_mem));
	ifbr(6 == PcutIxLen(&pcut.base, kP2OctetStringPartIxContent, os_mem));
	ifbr(8 == PcutIxLen(m, kPcutIxAll, whole));

	ifbr(0 == PcutIxOffset(m, kP2DatatypeWithContentPartIxDatatype, whole));
	ifbr(1 == PcutIxOffset(m, kP2DatatypeWithContentPartIxContent, whole));
	ifbr(8 == PcutIxOffset(m, kPcutIxAll, whole));

	ifer(PcutIxValid(m, kP2DatatypeWithContentPartIxDatatype, whole));
	ifer(PcutIxValid(m, kP2DatatypeWithContentPartIxContent, whole));
	ifer(PcutIxValid(m, kPcutIxAll, whole));
